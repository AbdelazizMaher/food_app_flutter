...
class CategoryFilterModel implements JsonFactory<CategoryFilterModel> {
  CategoryFilterModel({
      List<MealDetails>? meals,}){
    _meals = meals;
}

  CategoryFilterModel.fromJson(dynamic json) {
    if (json['meals'] != null) {
      _meals = [];
...
CategoryFilterModel copyWith({  List<MealDetails>? meals,
}) => CategoryFilterModel(  meals: meals ?? _meals,
);
  ...
  CategoryFilterModel fromJson(Map<String, dynamic> json) {
    return CategoryFilterModel.fromJson(json);
  }
